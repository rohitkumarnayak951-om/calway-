  ...
  return (
    <div className="w-full px-4 sm:px-8 lg:px-12 xl:px-16 2xl:px-20 mx-auto py-6 sm:py-10">
      ...
      <div className="flex items-center justify-between pb-6 border-b border-gray-200 mb-8">
        <div>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-gray-900 tracking-tight flex items-center gap-3.5">
            <span>Your Morning Basket</span>
            <span className="text-sm font-black px-3.5 py-1.5 bg-brand-100 text-brand-900 rounded-full">
              {items.length} {items.length === 1 ? 'item' : 'items'}
            </span>
          </h1>
          <p className="text-sm sm:text-base text-gray-600 font-medium mt-1.5">
        ...
        <button
          onClick={clearCart}
          className="text-sm font-black text-red-600 hover:text-red-700 flex items-center gap-1.5 hover:underline"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-brand-50/80 rounded-3xl p-4 sm:p-5 border border-brand-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3.5">
            <div className="flex items-center gap-3.5">
              <div className="p-3 bg-brand-600 text-white rounded-2xl shrink-0">
                <MapPin className="w-5 h-5" />
              </div>
              <div>
                <div className="text-sm sm:text-base font-black text-brand-950 flex items-center gap-2">
                  <span>Delivering to: {selectedLocation.name}</span>
                  <span className="text-xs bg-brand-200 text-brand-950 px-2 py-0.5 rounded-full font-mono font-bold">
                    {selectedLocation.pincode}
                  </span>
                </div>
                <div className="text-xs sm:text-sm font-semibold text-brand-700 mt-0.5">
            ...
            <button
              ...
              className="px-4 py-2 bg-white hover:bg-brand-100 text-brand-950 text-xs sm:text-sm font-black rounded-xl border border-brand-300 self-start sm:self-auto transition-colors"
            >
              Change Area
            </button>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm divide-y divide-gray-100 overflow-hidden">
            {items.map((item) => (
              <div
                ...
                className="p-4 sm:p-6 flex items-center justify-between gap-4 hover:bg-gray-50/40 transition-colors"
              >
                {/* Thumbnail & Product Details */}
                <div className="flex items-center gap-3.5 sm:gap-5 flex-1 min-w-0">
                  <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-2xl overflow-hidden bg-gray-50 border border-gray-100 shrink-0">
                    <Image
                  ...
                  <div className="truncate">
                    <Link
                      ...
                      className="font-black text-gray-900 text-sm sm:text-base lg:text-lg hover:text-brand-700 transition-colors truncate block"
                    >
                      {item.product.name}
                    </Link>
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="text-xs sm:text-sm font-black text-gray-800 bg-gray-100 px-2.5 py-0.5 rounded-lg">
                        {item.selectedWeight}
                      </span>
                ...
                <div className="flex items-center gap-3 sm:gap-6 shrink-0">
                  <div className="text-right">
                    <div className="text-base sm:text-lg lg:text-xl font-black text-gray-900">
                  ...
                  {/* Quantity Stepper */}
                  <div className="flex items-center bg-brand-50 rounded-xl p-1 border border-brand-200">
                    <button
                      onClick={() => updateQuantity(item.product.id, item.quantity - 1, item.selectedWeight)}
                      className="w-8 sm:w-9 h-8 sm:h-9 rounded-lg bg-white hover:bg-gray-100 flex items-center justify-center text-brand-900 transition-colors shadow-2xs"
                      aria-label="Decrease quantity"
                    >
                      <Minus className="w-3.5 h-3.5" />
                    </button>
                    <span className="w-8 text-center font-black text-sm sm:text-base text-brand-950">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() => updateQuantity(item.product.id, item.quantity + 1, item.selectedWeight)}
