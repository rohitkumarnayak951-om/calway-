  ...
  return (
    <section className="py-5 sm:py-8 overflow-hidden w-full max-w-full">
      <div className="w-full px-3 sm:px-8 lg:px-12 xl:px-16 2xl:px-20 mx-auto">
        ...
        <div className="flex items-center gap-2.5 sm:gap-4 overflow-x-auto no-scrollbar pb-2 pt-1 w-full max-w-full">
          {/* "All" button if interactive */}
          {onSelectCategory && (
            <button
              onClick={() => onSelectCategory('all')}
              className={`flex-shrink-0 flex flex-col items-center justify-center w-24 sm:w-32 p-2.5 sm:p-4 rounded-2xl sm:rounded-3xl border transition-all duration-200 text-center ${
            ...
            >
              <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-emerald-100 flex items-center justify-center text-xl sm:text-3xl mb-1.5 sm:mb-2">
                🧺
              </div>
              <span className="text-xs sm:text-base font-extrabold truncate w-full">All Veggies</span>
              <span className={`text-[10px] sm:text-xs mt-0.5 font-medium ${selectedCategory === 'all' ? 'text-brand-200' : 'text-gray-400'}`}>
                26 items
              </span>
            </button>
          )}
            ...
            if (onSelectCategory) {
              return (
                <button
                  ...
                  className={`flex-shrink-0 flex flex-col items-center justify-center w-28 sm:w-36 p-2.5 sm:p-4 rounded-2xl sm:rounded-3xl border transition-all duration-200 text-center group ${
                    isSelected
                      ? 'bg-brand-700 text-white border-brand-700 shadow-md scale-105'
                      : 'bg-white hover:bg-brand-50 border-gray-100 hover:border-brand-200 shadow-xs'
                  }`}
                >
                  <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gray-50 flex items-center justify-center text-xl sm:text-3xl mb-1.5 sm:mb-2 group-hover:scale-110 transition-transform">
                    {cat.icon}
                  </div>
                  <span className={`text-xs sm:text-base font-bold leading-tight line-clamp-1 ${isSelected ? 'text-white' : 'text-gray-900 group-hover:text-brand-800'}`}>
                    {cat.name.split('(')[0].trim()}
                  </span>
                  <span className={`text-[10px] sm:text-sm mt-0.5 font-semibold truncate w-full ${isSelected ? 'text-brand-200' : 'text-emerald-800'}`}>
                    {cat.bengaliName}
                  </span>
                </button>
              );
