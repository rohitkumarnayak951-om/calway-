...
export const Testimonials: React.FC = () => {
  return (
    <section className="py-8 sm:py-14 bg-white overflow-hidden w-full max-w-full">
      <div className="w-full px-3 sm:px-8 lg:px-12 xl:px-16 2xl:px-20 mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-6 sm:mb-12">
          ...
          <p className="text-xs sm:text-base lg:text-lg text-gray-600 mt-2 max-w-2xl mx-auto">
            Real feedback from households in Salt Lake, New Town, Ballygunge, and South Kolkata.
          </p>
        </div>

        {/* Testimonials Grid: Swipeable horizontal carousel on mobile, 3-col grid on desktop */}
        <div className="flex overflow-x-auto no-scrollbar snap-x snap-mandatory gap-3.5 pb-2 md:grid md:grid-cols-3 md:gap-8 w-full max-w-full">
          {testimonials.map((item) => (
            <div
              key={item.id}
              className="w-[84vw] max-w-[340px] shrink-0 snap-center md:w-auto md:max-w-none md:shrink bg-brand-50/40 rounded-3xl p-5 sm:p-8 border border-brand-100 flex flex-col justify-between relative hover:shadow-soft transition-all duration-200"
            >
              <div>
                {/* Stars & Quote Icon */}
                <div className="flex items-center justify-between mb-3.5 sm:mb-4">
                  ...
                  <Quote className="w-6 sm:w-8 h-6 sm:h-8 text-brand-200" />
                </div>

                <p className="text-xs sm:text-base text-gray-700 leading-relaxed italic">
                  &ldquo;{item.text}&rdquo;
                </p>
              </div>

              <div className="mt-5 sm:mt-6 pt-3.5 sm:pt-4 border-t border-brand-100/80 flex items-center justify-between">
                <div>
                  <h4 className="font-extrabold text-sm sm:text-base text-gray-900 flex items-center gap-1.5">
                    {item.name}
                    <CheckCircle2 className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-brand-600 fill-brand-100" />
                  </h4>
                  <p className="text-xs sm:text-sm text-brand-800 font-medium">{item.area}</p>
                </div>
                <span className="text-[10px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 bg-white text-brand-900 border border-brand-200 rounded-lg shadow-2xs">
                  {item.dish}
                </span>
              </div>
            </div>
